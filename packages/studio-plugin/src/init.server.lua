--!strict
--[[
	Studio Hub Plugin (UI Shell)
	Connects to Runtime injected via 'hub open' command.
	All business logic lives in the Runtime module.
]]
...
-- Try to require Runtime from ReplicatedStorage
...
Runtime.onStatusChange = function(status: string, color: Color3?)
	updateStatus(status, color)
end

Runtime.onConnected = function(studioId: string)
	isConnected = true
	updateId(studioId)
	connectButton.Text = "Disconnect"
end

Runtime.onDisconnected = function()
	isConnected = false
	updateId(nil)
	connectButton.Text = "Connect"
end
